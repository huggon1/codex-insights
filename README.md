...
- generate seven narrative sections in parallel (project_areas,
  ...
  summary
- render Markdown or self-contained HTML reports with all eight
  sections plus stats, session snapshots, and run metadata
...
`build-report` and `extract-session-facets` print progress to stderr by
default so long-running LLM analysis is observable without polluting
report stdout. Pass `--quiet` to suppress progress logs.
...
`build-report` additionally accepts `--format markdown|html` to select
the report renderer.
...
into the renderer so you can iterate on Markdown or HTML layout without
spending LLM calls. It is a rendering harness only — every PR that
changes prompts or analysis logic is verified end to end with the real
SDK path (see Testing below).
...
Markdown/HTML rendering. They use a fake codex client and never call
the real SDK.
...
- Live `codex exec --json` is the only model path; line-diff metrics,
  subjective satisfaction/helpfulness facets, and true branch-aware
  retry deduplication are deliberately deferred.
